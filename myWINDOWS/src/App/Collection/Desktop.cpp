...
DesktopWindow::DesktopWindow(Desktop& app, int _id, std::wstring _name, Pos _pos, Size _size)
	: app(app), Window(_id, _name, _pos, _size)
{
	Window::getCanvas().setBackground(background);

	PUSH_ELEMENTS(bPainter, Button(L"  Painter  ", { 73, 4 }, true));
	PUSH_ELEMENTS(bChess, Button(L"   Chess   ", { 73, 9 }, true));
