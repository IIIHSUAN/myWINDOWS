...
void Canvas::renderWith(Canvas & front)
{
	...
	for (int i = -my; i < h && w > 0; i++)
		_renderLine(index(i + y, x) - mx, w + mx, front.getRaw(), i * MY_WINDOW_WIDTH - mx);  //canvas.replace(index(i + y, x) - mx, w + mx, front.getRaw(), i*MY_WINDOW_WIDTH - mx, w + mx);
}
void Canvas::renderCharImage(CharImage & src, bool isWhitespace)
{
	...
	for (int i = 0; i < h; i++)
		_renderLine(index(i + y, x), w, src.rawData, i * (src.size.width - 1), isWhitespace);
		/*canvas.replace(
			index(i + y, x), w,
			src.rawData,
...
void Canvas::renderWindow(Window& window)  // with custom size
{
	...
	for (int i = -my; i < h && w > 0; i++)
		_renderLine(index(i + y, x) - mx, w + mx, window.getCanvas().rawData, i * MY_WINDOW_WIDTH - mx);  //canvas.replace(index(i + y, x) - mx, w + mx, window.getRaw().canvas, i*MY_WINDOW_WIDTH - mx, w + mx);
}
